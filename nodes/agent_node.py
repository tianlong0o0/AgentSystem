...
small_llm = LLM(init_msg="你是一个执行搜救任务的人工智能助手，"
                "请根据信息判断附近是否有被困人员。")
large_llm = LLM(model=MODEL_MAX_VL,
                init_msg="你是一个执行搜救任务的人工智能助手，"
                "请根据信息判断附近是否有被困人员，"
                "如果有，"
                "请从操作库中选择需要执行的操作(每次只可选择1种操作)。")
...
async def observe(img_queue: asyncio.Queue, action_queue: asyncio.Queue,
                  detected_classes: set, feedback_queue: asyncio.Queue):
    """
    识别场景中的异常并判断是否与任务有关
    Args:
        img_queue:相机图像队列
    ...
    if not new_detected_classes <= detected_classes:
        answer = small_llm.call("参考YOLO模型的识别结果"
                                "判断附近是否可能有被困人员，"
                                "请回答'有'或'没有'。", result)
...
async def make_decision(img_queue: asyncio.Queue, action_queue: asyncio.Queue,
    ...
    while True:
        if "没有" in answer:
            ...
            answer = large_llm.call("请选择需要执行的操作。"
                                    "(可执行操作包含:'移动至被困人员处',"
                                    "'在被困人员物品周围搜寻被困人员',"
                                    "'通知总部找到被困人员',"
                                    "'向被困人员发放紧急救援物资',"
            ...
            while True:
                if "在被困人员物品周围搜寻被困人员" in answer:
                    ...
                    answer = large_llm.call("未从回答中识别到可执行操作，"
                                            "请输出正确的需要执行的操作。"
                                            "(可执行操作包含:"
                                            "'移动至被困人员处',"
                                            "'在物品周围搜寻被困人员',"
...
async def main(img_queue: asyncio.Queue, action_queue: asyncio.Queue,
    ...
    while True:
        detected_classes = await observe(img_queue, action_queue,
                                         detected_classes, feedback_queue)
