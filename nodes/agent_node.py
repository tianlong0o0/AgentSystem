...
large_llm = LLM(model=MODEL_MAX_VL, init_msg="你是一个执行搜救任务的人工智能助手，请根据信息判断附近是否有被困人员，如果有，请从操作库中选择需要执行的操作(每次只可选择1种操作)。")
...
async def observe(img_queue: asyncio.Queue, action_queue: asyncio.Queue, feedback_queue: asyncio.Queue):
    ...
    while True:
        ...
        answer = small_llm.call("参考YOLO模型的识别结果判断附近是否可能有被困人员，请回答'有'或'没有'。", image)
        if "没有" in answer:
            break
        elif "有" in answer:
            ...
            answer = small_llm.call("格式输出错误，请回答'有'或'没有'。", image)
